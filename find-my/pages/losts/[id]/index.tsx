    ...
    <>
      <div>
        {data?.lost?.photos[0]?.file ? (
          <img src={CFImageUrl(data?.lost?.photos[0]?.file)} className="w-full h-96 bg-slate-500" />
        ) : (
              ...
              <Link href={`/users/profiles/${data?.lost?.user?.id}`}>
                <a>
                  {data?.lost?.user?.avatar ? (
                    <img src={CFImageUrl(data?.lost?.user?.avatar)} className="w-12 h-12 rounded-full bg-green-500" />
                  ) : (
